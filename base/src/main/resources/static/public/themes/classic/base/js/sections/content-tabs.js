(function (window, document, $) {
    ...
    $.site.contentTabs = {
        ...
        bind: function () {
            ...
            $navContabs.on('dblclick.site.contabs','.contabs-scroll li',function () {
                var tabUrl = $(this).find('a').attr('href');
                // 刷新当前页
                self.refreshIframe(tabUrl);
            });
        ...
        ifameTabs: function (el) { // 为当前doc对象中的a[data-iframe]标签绑定创建标签页功能
            ...
            $(doc).on('click', 'a[data-iframe]', function (e) {
                var $item = $(this), urlResult,
                    url = $item.attr('href'),
                    title = $.trim($item.data('title')) || $.trim($item.attr('title')) || $.trim($item.text()),
                    activeMenu = $.trim($item.data('activemenu')) || '',
                ...
                if (tabResult && iframeContent === '#admui-pageContent') {
                    ...
                    self.tabType = true;
                    self.buildTab({name: title, url: url, activeMenu: activeMenu});
        ...
        enable: function ($el) { // 左侧菜单定位
            ...
            var href = $.trim(tabItem.attr('href')),
                _result = href.indexOf('#'),
                tabUrl = _result > 0 ? href.substring(0, _result) : href,
                indexOf = tabUrl.indexOf("?"),
                tabUrl = indexOf > 0 ? tabUrl.substring(0, indexOf) : tabUrl,
                $current = $instance.find('a[href="' + tabUrl + '"]'),
                $currentParents, $currentLi, $currentHasSub, $instanceLi, checkedId, currentId;

            if ($current.length === 0) {
                var activeMenuUrl = tabItem.data('activemenu') || '';
                if (activeMenuUrl != '') {
                    $current = $instance.find('a[href="' + activeMenuUrl + '"]');
                }
            }
        ...
        buildTab: function (opt) { // 新建标签页
            ...
            var activeMenu = '';
            if (opt.hasOwnProperty('activeMenu')) {
                activeMenu = opt.activeMenu;
            }
            ...
            $tabNav.append('<li  class="active"><a href="' + tabUrl + '" target="' + tabName +
                '" title="' + opt.name + '' + '" data-activemenu="' + activeMenu + '" rel="contents"><span>' + opt.name + '</span><i class="icon' +
        ...
        refreshIframe: function (tabUrl, refreshType) {
            if (refreshType == undefined) {
                refreshType = 1;
            }
            // 刷新当前页
            var iframelist = this.$content.children('[src="' + tabUrl + '"]');
            if (iframelist.length > 0) {
                var currentIframe = iframelist[0];
                $('.page-loading').show();//显示loading
                currentIframe.onload = function(){
                    $('.page-loading').hide();
                };
                if (refreshType == 1) {
                    currentIframe.contentWindow.location.reload(true);
                } else {
                    iframelist.attr('src', tabUrl);
                }
            }
        },
        refreshActiveTab: function (refreshType) {// 刷新当前Tab, $.site.contentTabs.refreshActiveTab();
            var $navContabs = $(".site-contabs");
            var $navLi = $navContabs.find('ul.con-tabs>li.active>a'),
                tabUrl = $navLi.attr('href');

            //刷新
            this.refreshIframe(tabUrl, refreshType);
        },
        closeActiveTab: function () {// 关闭当前Tab, $.site.contentTabs.closeActiveTab();
            var self = $.site.contentTabs,
                $navContabs = $(".site-contabs");

            var currItem = $navContabs.find("li.active");
            this.closeTab(currItem);
        },
        gotoTab: function (tabName, tabUrl, refreshType) {// 跳转到指定的Tab（不存在则创建, 存在则刷新）, $.site.contentTabs.gotoTab('模版管理', 'card/template/list');
            this.buildTab({name: tabName, url: tabUrl});
            //刷新
            this.refreshIframe(tabUrl, refreshType);
        },
        redirectTab: function (tabName, tabUrl, refreshType) {// 跳转到指定的Tab并关闭当前Tab（不存在则创建, 存在则刷新）, $.site.contentTabs.redirectTab('模版管理', 'card/template/list');
            this.closeActiveTab();
            this.buildTab({name: tabName, url: tabUrl});
            //刷新
            this.refreshIframe(tabUrl, refreshType);
