    ...
    $.extend($.site, {
        theme: function () { // 主题渲染
            ...
            if (themeColor && themeColor !== 'primary') {
                setTimeout(function () {
                    $parentLink.attr('href', $.ctx + '/themes/classic/base/skins/' + themeColor + '/index' + parentEtx + '.css');
                }, 130);
            }
        ...
        iframeTheme: function () { // 子框架主体内容部分主题
            ...
            if (themeColor && themeColor !== 'primary') {
                $iframeLink.attr('href', $.ctx + '/themes/classic/base/skins/' + this.themeColor + '/site' + iframeEtx + '.css');
            }
        },
        _tabsDraw: function () { // 标签页重绘
            ...
            for (var key in settings) {
                ...
                tabName = tab.name;
                var activeMenu = '';
                if (tab.hasOwnProperty('activeMenu')) {
                    activeMenu = tab.activeMenu;
                }
                tabTpl = '<a href="' + tabUrl + '" ' + 'target="' + key + '"' +
                    '" title="' + tabName + '' + '" data-activemenu="' + activeMenu + '" rel="contents"><span>' + tabName + '</span><i class="icon' +
        ...
        _urlRequest: function (url) { // 处理存储信息中没有的页面访问（创建新的标签页）
            ...
            var _result = url.indexOf('?'),
                comparisonUrl = _result > 0 ? url.substring(0, _result) : url;
            $('.site-menu a').each(function () {
                var $item = $(this);

                if ($item.attr('href') === comparisonUrl) {
        ...
        iframeEvents: function ($el) { // 子框架主体内容事件处理
            //显示loading
            $('.page-loading').show();
            setTimeout(function(){$('.page-loading').hide();}, 10000);
